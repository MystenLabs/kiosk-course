...
if [ -f .env ] && [ -f .nft.env ] && [ -f .admin.kiosk.env ] && [ -f .transfer_policy.env ] && [ -f .rules.env ]; then
    source .env
    source .nft.env
    ...
    echo "No .env, .nft.env, .admin.kiosk.env, .transfer_policy.env or .rules.env file found"
    exit 1
fi
